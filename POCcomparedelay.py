...
def read_file(fpga_in_file, fpga_out_file):
    ...
    with open(fpga_in_file, 'r') as fpga_in_data:
        while True:
            ...
            if not fpga_line:
                break
            total_fpga_in_data.append(fpga_line[-20:-1])
        fpga_in_data.close()
...
def calculator(fpga_in_file, fpga_out_file): # fpgain = poc ,fpgaout = hy
...
if __name__ == '__main__':
    server_file = r'D:\延时测试\上海对比3+1绑核\ENTRUST_BIN_0829'
    clientdemo_file = r'D:\延时测试\上海对比3+1绑核\SSE_ENTRUST_BIN_0829'
    calculator(server_file, clientdemo_file) # server_file = poc ,fpgaout = hy

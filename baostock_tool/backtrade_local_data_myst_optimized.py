...
# 创建QuestDB客户端实例（在并发遍历之前创建，避免重复创建）
questdb_client = create_questdb_client()
...
def get_stock_data_from_db(stock_code="601288", market="sh", start_date="2019-01-01", stock_data_dict=None):
    """
    获取单只股票的K线数据
    ...
    Args:
        ...
        stock_data_dict (dict): 预加载的股票数据字典（可选）

    Returns:
        DataFrame: 股票K线数据
    """
    # 如果提供了预加载的数据字典，从内存中获取
    if stock_data_dict is not None and stock_code in stock_data_dict:
        df = stock_data_dict[stock_code].copy()
        # 过滤日期范围
        start_dt = pd.to_datetime(start_date)
        adjusted_start_dt = start_dt - pd.DateOffset(days=365)
        df = df[df.index >= adjusted_start_dt]
        return df
    ...
    # 数据清洗与格式化
    # 转换日期并去除时区信息，避免tz-naive和tz-aware比较错误
...
def get_all_stock_data_from_db(stock_codes_df, start_date):
    """
    批量获取所有股票的K线数据（全量查询，减少数据库访问次数）

    Args:
        stock_codes_df (DataFrame): 股票列表DataFrame
        start_date (str): 回测开始日期
    ...
    logger.info(f"开始全量查询股票K线数据，时间范围: {adjusted_start_date} 至今...")
    ...
    df_all = questdb_client.query(query)
    logger.info(f"全量查询完成，共获取 {len(df_all)} 条记录")
    ...
    # 按股票代码分组，存储到字典
    stock_data_dict = {}
    for code_int, group_df in df_all.groupby('code_int'):
        group_df = group_df.set_index('date')
        group_df = group_df.sort_index()
        group_df = group_df.ffill()  # 前向填充缺失值
        stock_data_dict[str(int(code_int))] = group_df

    logger.info(f"数据已按股票代码分组，共 {len(stock_data_dict)} 只股票")
...
def run_backtest(stock_code, market, name, start_date, end_date, strategy_class=SimpleTrendStrategy, save_to_db=False, printlog=True, stock_data_dict=None):
    """
    执行单只股票的回测
    ...
    try:
        # 获取股票数据（优先使用预加载数据）
        df = get_stock_data_from_db(stock_code, market, start_date, stock_data_dict)
...
def _backtest_single_stock_worker(args):
    ...
    Args:
        args: (stock_code, market, name, start_date, end_date, strategy_class, stock_data_dict)
    """
    stock_code, market, name, start_date, end_date, strategy_class, stock_data_dict = args
    try:
        # 并发模式下关闭详细日志以提高性能
        result = run_backtest(stock_code, market, name, start_date, end_date, strategy_class, save_to_db=False, printlog=False, stock_data_dict=stock_data_dict)
...
def batch_backtest(start_date, end_date, strategy_class=SimpleTrendStrategy, save_to_db=False):
    ...
    # ============ 全量查询所有股票K线数据（优化：减少数据库访问次数）============
    stock_data_dict = get_all_stock_data_from_db(stock_codes, start_date)
    logger.info(f"股票数据预加载完成，内存中共 {len(stock_data_dict)} 只股票数据")

    # 存储所有回测结果（多进程模式下无需锁）
    all_results = []
    summary_file = f"csv/backtest_summary_{start_date.replace('-', '')}_to_{end_date.replace('-', '')}.csv"

    # 准备股票参数列表（只传递当前股票需要的数据，减少内存占用）
    stock_params = [
        (str(index), row['market'], row['name'].replace('*', ''), start_date, end_date, strategy_class, 
         {str(index): stock_data_dict.get(str(index))} if str(index) in stock_data_dict else {})

...
class QuestDBClient:
    ...
    def execute(self, sql, timeout=120):
        """
        执行SQL语句（用于INSERT等操作）
...
class TimeBasedBacktester:
    """
    按时间遍历的回测引擎 - QuestDB版本

    支持策略模块化，可以通过 strategy 参数注入不同的策略
    股票历史数据从QuestDB加载，其他数据（交易日历、触发点位）从MySQL加载
    """
    ...
    def run_backtest(self, strategy_name='TimeBasedStrategy', save_to_db=False):
        ...
        logger.info(f"{'=' * 60}")
        logger.info(f"开始按时间遍历回测 (QuestDB数据源)")
        ...
        # 保存到CSV
        summary_file = f"csv/time_based_backtest_questdb_{self.config['start_date'].replace('-', '')}_to_{self.config['end_date'].replace('-', '')}.csv"
        pd.DataFrame(self.trading_records).to_csv(summary_file, index=False, encoding='utf-8-sig')
        logger.info(f"交易记录已保存至: {summary_file}")

        daily_file = f"csv/time_based_daily_values_questdb_{self.config['start_date'].replace('-', '')}_to_{self.config['end_date'].replace('-', '')}.csv"

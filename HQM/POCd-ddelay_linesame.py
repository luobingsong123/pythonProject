...
def calculator(fpga_in_file, fpga_out_file):
    ...
    while i < len(total_fpga_in_data):
        try:
            ...
            if 0< delay < 198765:
                ...
                total_fpga_out_data_jg.append(int(total_fpga_out_data[i]))
                if len(all_delay) > 1:
                    jg = total_fpga_out_data_jg[-1] - total_fpga_out_data_jg[-2]
                    if jg < 5000:
                        delay_log.write(str(total_fpga_out_data[i]) + ',' + str(int(total_fpga_in_data[i])) + ',' + str(delay) + ',' + str(jg) + '\n')
                        count += 1
                        all_jg += jg
                        count_jg += 1
                    else:
                        delay_log.write(
                            str(total_fpga_out_data[i]) + ',' + str(int(total_fpga_in_data[i])) + ',' + str(delay) + ',' + str(all_delay[-1]) + '\n')
                        all_sb += delay
                        count_sb += 1
                else:
                    delay_log.write(
    ...
    #             # print(delay)
    #             if len(all_delay) > 1:
    #                 p = all_delay[-1] - all_delay[-2]
    #                 total_delay += delay
    #                 delay_log.write(
    #                     str(total_fpga_in_data[i]) + ',' + str(total_fpga_out_data[j]) + ',' + str(delay) + '\n')
    #                 # print(ap)
    #                 if p > 2000:
    #                     all_delay = []
    #                     i += 1
    ...
    #             else:
    #                 total_delay += delay
    #                 delay_log.write(str(total_fpga_in_data[i]) + ',' + str(total_fpga_out_data[j]) + ',' + str(delay) + '\n')
    #                 j += 1
    #         elif delay >= 60000:
    #             i += 1
    #         else:
...
if __name__ == '__main__':
    ...
    # server_file = r'D:\延时测试\深圳旁路\端到端\sz_tms_source_index.txt'
    # clientdemo_file = r'D:\延时测试\深圳旁路\端到端\SZSE_INDEX_BIN_0827'
    # server_file = r'D:\延时测试\深圳旁路\端到端\sz_tms_source_turnover.txt'
    # clientdemo_file = r'D:\延时测试\深圳旁路\端到端\SZSE_TURNOVER_BIN_0827'
    calculator(server_file, clientdemo_file)

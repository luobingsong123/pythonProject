...
def rebalance(context, signal):
    ...
    if (len(df_01['close'].dropna()) != 0 and len(df_02['close'].dropna()) != 0):
        t = min(int(t1['volume'].values / 10), int(t2['volume'].values / 10), int((context.portfolio.total_value / 2) / (300 * g.futures_margin_rate * df_01['close'].values)),
                int((context.portfolio.total_value / 2) / (300 * g.futures_margin_rate * df_02['close'].values)), (g.count))
        print('t:%d' % t)
        # 根据signal值选择建仓或者平仓
...
def get_current_month_future(context, symbol):
    ...
    if dt.day <= third_friday_date or (
            dt.day > third_friday_date and not any([datetime.date(dt.year, dt.month, third_friday_date + i) in get_all_trade_days() for i in range(dt.day - third_friday_date)])):
        year = str(dt.year)[2:]
        month = str(dt.month)
    else:
